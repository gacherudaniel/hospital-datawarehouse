"""
Interactive OLAP Operations Dashboard (SQL Data Warehouse Version)
Hospital Data Warehouse - St. Bonaventure Hospital
Group Assignment - Data Analytics

Run with: streamlit run streamlit_olap_sql.py

This version reads from SQLite data warehouse instead of CSV files
"""
...
def get_database_connection():
    ...
    return sqlite3.connect('hospital_datawarehouse.db', check_same_thread=False)

# Cache data loading
@st.cache_data
def load_data_from_warehouse():
    """Load and prepare data from SQL warehouse"""
    conn = sqlite3.connect('hospital_datawarehouse.db')
    
    # Load using the pre-built view for better performance
    query = """
        SELECT 
            encounter_id,
            date,
            ...
            service_name,
            service_category
        FROM vw_encounter_details
    """
    
    data = pd.read_sql_query(query, conn)
    
    # Load dimension tables for metrics
    dim_patients = pd.read_sql_query("SELECT * FROM dim_patients", conn)
    dim_doctors = pd.read_sql_query("SELECT * FROM dim_doctors", conn)
    dim_services = pd.read_sql_query("SELECT * FROM dim_services", conn)
    ...
    return data, dim_patients, dim_doctors, dim_services

# Load data
data, dim_patients, dim_doctors, dim_services = load_data_from_warehouse()

# Header
st.title("🏥 Hospital Data Warehouse - OLAP Operations")
st.markdown("### St. Bonaventure Hospital Analytics Dashboard")
st.markdown("*Connected to SQL Data Warehouse (SQLite)*")
st.markdown("---")
...
if operation == "📈 Overview":
    ...
    with col2:
        ...
        st.metric("Encounters", f"{len(data):,}")
...
ORDER BY year, month;"""
        ...
        sql_query = """
SELECT strftime('%Y', date) as year,
       CAST((CAST(strftime('%m', date) AS INT) + 2) / 3 AS INT) as quarter,
       SUM(total_bill_ksh) as revenue,
       COUNT(*) as encounters
FROM fact_encounters
...
GROUP BY wing;""", language="sql")
        ...
        with st.expander("📝 View SQL Query"):
            st.code("""
SELECT wing, payment_method,
       SUM(total_bill_ksh) as revenue,
       COUNT(*) as encounters
FROM fact_encounters
GROUP BY wing, payment_method;""", language="sql")
...
GROUP BY f.wing, f.payment_method, d.specialty;""", language="sql")
        ...
        with st.expander("📝 View SQL Query"):
            where_clause = []
            if selected_wing != 'All':
                where_clause.append(f"wing = '{selected_wing}'")
            if selected_payment != 'All':
                where_clause.append(f"payment_method = '{selected_payment}'")
            if selected_specialty != 'All':
                where_clause.append(f"specialty = '{selected_specialty}'")
            
            where_str = "WHERE " + " AND ".join(where_clause) if where_clause else ""
            
            st.code(f"""
SELECT * FROM vw_encounter_details
{where_str}
LIMIT 100;""", language="sql")

# Copy the rest of the operations from the original file...
# (SLICE, DICE, PIVOT sections remain the same)
...
# Footer
st.markdown("---")
st.markdown("**Group Assignment - Data Analytics** | St. Bonaventure Hospital Data Warehouse | SQLite Backend")
